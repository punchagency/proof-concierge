...
function AudioDeviceSelector() {
  ...
  const handleDeviceChange = useCallback(
    async (type: "input" | "output", deviceId: string) => {
      try {
        if (!daily) return;
        ...
        try {
          ...
          if (inputDevices && typeof inputDevices === "object") {
            ...
            if (audioInputId) {
              ...
              const defaultInputDevice = inputs.find(
            ...
            if (audioOutputId) {
              ...
              const defaultDevice = outputs.find(
                (d) => d.deviceId === "default" || d.deviceId === ""
              );
              if (defaultDevice) {
              ...
              audioInputDevices.map((device) => (
                ...
                >
                  ...
                  {selectedInputDevice === device.deviceId && (
                    <Badge
                      variant="default"
                      className="ml-auto py-0 px-1 h-4 text-[10px]"
                    >
                      Active
                    </Badge>
                  )}
...
export function CallUI({ onLeave }: CallUIProps) {
            ...
            if (daily && !isMuted) {
              ...
              if (statsData?.audio?.send) {
                ...
                if (audioStats.bitrate === 0 && !isMuted) {
                  console.warn(
                    "Warning: Audio bitrate is 0 - your audio may not be transmitting"
                  );
                }
              }
            }
  ...
  useEffect(() => {
    ...
    const checkAudioTransmission = () => {
      try {
        ...
        const localParticipant = daily.participants()
          .local as unknown as DailyParticipantWithAudio;
        ...
        if (audioActivity < 5 && lastAudioActivity < 5) {
          ...
          if (noAudioCount === 20 && !isMuted) {
            console.warn(
              "No audio activity detected for 10 seconds while unmuted. Consider checking your microphone."
            );
          }
        }
  ...
  const toggleScreenShare = async () => {
    ...
    if (!isFullyJoined) {
      blueToast(
        "Please wait until call is fully connected before sharing your screen",
        {},
        "error"
      );
      return;
    ...
    try {
      ...
      const errorMessage =
        (error as Error)?.message || "Failed to toggle screen sharing";
              ...
              >
                {isFullscreen ? (
                  <>
                    ...
                    <div
                      className="absolute -z-10 w-[300px] h-[300px] -bottom-[150px] -right-[150px] bg-blue-400/5 rounded-full blur-3xl opacity-50 blob-animation"
                    ...
                    <p className="text-gray-500 text-sm max-w-[300px] mx-auto">
                      You can share your screen using the controls below or wait
