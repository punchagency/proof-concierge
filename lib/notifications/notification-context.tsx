...
import socketService, { Notification, NotificationType } from '@/lib/websocket/socket-service';
import { requestFCMPermission, onMessageListener, registerServiceWorker } from '@/lib/firebase/firebase-config';
    ...
    const setupMessageListener = async () => {
      try {
        ...
        if (payload) {
          ...
          if (data?.type === 'query_status_changed') type = 'queryStatusChanged';
          ...
          const newNotification: Notification = {
            type,
            queryId: data?.queryId ? parseInt(data.queryId as string) : undefined,
            messageId: data?.messageId ? parseInt(data.messageId as string) : undefined,
  ...
  const showNotificationToast = (type: NotificationType, data: any) => {
    switch (type) {
      ...
      case 'queryStatusChanged':
        blueToast('Query Status Changed', {
      ...
      case 'callRequested':
        blueToast('Call Requested', {
          description: `A ${data.mode || 'video'} call was requested for query #${data.queryId}`,
          action: {
            label: 'View',
            onClick: () => router.push(`/donor-queries/${data.queryId}`),
          },
